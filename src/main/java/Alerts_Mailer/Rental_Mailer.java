...
public class Rental_Mailer {
    ...
    private By propertyTypeLocator = By.xpath("//strong[text()='Property:']/parent::p");

    private By locationLocator = By.xpath( "(//span[contains(@style,'font-size:14px') and contains(@style,'color:#4a6075')])[1]");

    private By viewDetailCTALocator = By.xpath("//a[normalize-space()='View Property Details']");

    // 🔹 Constructor
    public Rental_Mailer(WebDriver driver) {
        this.driver = driver;
    ...
    public PropertyRedirectionResult clickViewDetailCTA() {
        ...
        try {
            ...
            String fullText = typeEle.getText();
            // Extract ONLY first line
        	String firstLine = fullText.split("\n")[0];
        	// Remove label
        	String propertyType = firstLine.replace("Property:", "").trim();

            // 📍 Get Location
//            WebElement locationEle = utils.find(locationLocator);
            String secondLine = fullText.split("\n")[1];
            String location = secondLine.replace("Location:", "").trim();
